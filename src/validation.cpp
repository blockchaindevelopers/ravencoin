...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            bool is_spent = false;
            if (AreAssetsDeployed()) {
                is_spent = SpendCoinWithAssets(inputs, txin.prevout, &txundo.vprevout.back(), assetCache); /** RVN START */ /* Pass assetCache into function */ /** RVN END */
            } else {
    ...
    // add outputs
    if (AreAssetsDeployed()) {
        AddCoinsWithAssets(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** RVN START */ /* Pass assetCache into function */ /** RVN END */
    } else {
        // add outputs
...
//                                      << ", " << i << ", " << hash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                bool is_spent = false;
                if (AreAssetsDeployed()) {
                    is_spent = SpendCoinWithAssets(view, out, &coin, &tempCache); /** RVN START */ /* Pass assetsCache into the SpendCoin function */ /** RVN END */
                } else {
...
static bool RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params, CAssetsCache* assetsCache = nullptr)
{
    ...
    for (const CTransactionRef& tx : block.vtx) {
        if (!tx->IsCoinBase()) {
            for (const CTxIn &txin : tx->vin) {
                if (AreAssetsDeployed()) {
                    SpendCoinWithAssets(inputs, txin.prevout, nullptr, assetsCache); /** RVN START */ /* Pass assetCache into function */ /** RVN END */
                } else {
                    inputs.SpendCoin(txin.prevout, nullptr);
                }
            }
        }
        // Pass check = true as every addition may be an overwrite.
        AddCoinsWithAssets(inputs, *tx, pindex->nHeight, pindex->GetBlockHash(), true, assetsCache);
    }
    return true;
}
